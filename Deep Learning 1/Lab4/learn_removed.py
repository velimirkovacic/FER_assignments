...
import time
import torch.optim
from unos import MNISTMetricDataset
...
if __name__ == '__main__':
    ...
    ds_train_removed = MNISTMetricDataset(mnist_download_root, split='train', remove_class=0)
    ...
    for epoch in range(epochs):
        print(f"Epoch: {epoch}")
        t0 = time.time_ns()
        train_loss = train(model, optimizer, train_loader_removed, device)
        ...
        torch.save(model, 'models/model_removed_' + str(epoch + 1) + 'ep.pt')